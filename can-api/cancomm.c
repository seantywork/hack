...
int InitCanComm(char CANDevName[MAX_CAN_DEV_NAME], uint8_t CANNodeID){


    TFP = fopen("/tmp/cansocklog", "a");

    fdebugLog("initiating...");


    strncpy(CAN_DEV_NAME, CANDevName, MAX_CAN_DEV_NAME);

    CAN_NODE_ID = CANNodeID;

    int ret = pthread_create(&SOCK_PTID, NULL, &cancomm_listener, NULL); 
...
int InitCmdGateway(){
    ...
    strcpy(address.sun_path, "/tmp/CO_command_socket");
...
int CmdGatewayASCII(char in[MAX_CAN_CMD_IN], char out[MAX_CAN_CMD_OUT]){
...
int strncpy_process(char src[MAX_CAN_CMD_OUT], char dst[MAX_CAN_CMD_OUT]){
 ...
 * Mainline thread
 ******************************************************************************/
void* cancomm_listener (void* varg) {




    fdebugLog("entered listener");
    ...
    char CANdevice[MAX_CAN_DEV_NAME] = {0};         /* CAN device, configurable by arguments. */
    ...
    fdebugLog("logging also to stdout");

    /* configure system log */
    setlogmask(LOG_UPTO (LOG_DEBUG)); /* LOG_DEBUG - log all messages */
    openlog("agvcancomm", LOG_PID | LOG_PERROR, LOG_USER); /* print also to standard error */

#ifdef AGVCANCOMM_CMDLINE_ENABLE
    ...
    fdebugLog("initiating using passed arguments");

    fprintf(TFP, "passed can device name: %s\n", CAN_DEV_NAME);
    fflush(TFP);

    fprintf(TFP, "passed can node id: %d\n", CAN_NODE_ID);
    fflush(TFP);   

    strncpy(CANdevice, CAN_DEV_NAME, MAX_CAN_DEV_NAME);
    CANptr.can_ifindex = if_nametoindex(CANdevice);
    mlStorage.pendingNodeId = CAN_NODE_ID;
    commandInterface = CO_COMMAND_IF_LOCAL_SOCKET;
    localSocketPath = "/tmp/CO_command_socket";

    fprintf(TFP, "can device name: %s\n", CANdevice);
    fflush(TFP);

    fprintf(TFP,"can if index: %d\n", CANptr.can_ifindex);
    fflush(TFP);

    fprintf(TFP, "localsockpath: %s\n", localSocketPath);
    fflush(TFP);
