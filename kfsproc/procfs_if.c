...
MODULE_DESCRIPTION("LKP-2 book:user_kernel_comm/procfs_if: simple procfs interfacing demo");
MODULE_LICENSE("Dual MIT/GPL");
MODULE_VERSION("0.1");

#define	OURMODNAME		"procfs_if"
...
static int __init procfs_if_init(void)
{
	int stat = 0;
...
static void __exit procfs_if_cleanup(void)
{
	gdrvctx->power = 0;
	remove_proc_subtree(OURMODNAME, NULL);
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 5, 0)
...
module_init(procfs_if_init);
module_exit(procfs_if_cleanup);